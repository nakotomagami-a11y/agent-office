...
/**
 * The app shell. There is no fixed chrome overlay — the project tabs / Docs /
 * theme / account chip render in normal flow at the top of `<main>` (see
 * `main-top-bar.tsx`), so no spacer row is needed here.
 *
 ...
 * position) everywhere — they're app-shell chrome, not a per-page effect.
 * Rendered here once so they show through every route's transparent
 * sidebar/main flow containers, instead of being (re)implemented — or
 * forgotten — per page.
 */
  ...
  return (
    ...
    >
      <div
        ...
        style={{ background: "radial-gradient(circle at 50% 50%, color-mix(in srgb, var(--acc) 16%, transparent), transparent 62%)" }}
      />
      <div
        aria-hidden
        className="absolute right-[-160px] top-[180px] w-[620px] h-[520px] pointer-events-none"
        style={{ background: "radial-gradient(circle at 50% 50%, color-mix(in srgb, var(--cyan) 9%, transparent), transparent 64%)" }}
      />
