...
function TrafficDot({ kind, onClick, label }: { kind: "close" | "min" | "max"; onClick: () => void; label: string }) {
  ...
  // Deliberately no `data-tauri-drag-region` attribute here, not even ="false"
  // — on older Tauri 2.x builds its mere presence (any value) makes the dot a
  // drag handle and swallows the click. Omitting it entirely works everywhere.
  return (
    <button
      type="button"
      aria-label={label}
