...
export type OfficeBuildToolbarProps = {
  ...
  /** The fill tool's own paint color — see `apply-cell-click.ts`'s
   *  `fillColorRef` doc for why this is separate from `grassColor`. */
  fillColor: GrassColor;
  onToggle: () => void;
  onSelectTool: (next: BuildTool | null) => void;
  onSelectGrassColor: (next: GrassColor) => void;
  onSelectFillColor: (next: GrassColor) => void;
  canUndo: boolean;
  canRedo: boolean;
  onUndo: () => void;
  onRedo: () => void;
...
const TOOL_HINT: Record<string, string> = {
  ...
  fill: "click water to flood it, or a platform to shade it",
};
      ...
      <AnimatePresence>
        {!active && (
          <motion.button
            key="build-entry"
            type="button"
            className={`build-entry-btn absolute z-[6] right-[16px] bottom-[16px] inline-flex items-center gap-[8px] px-[20px] py-[12px] rounded-[15px] text-[13.5px] font-bold cursor-pointer ${ACCENT_BTN}`}
            onClick={onToggle}
        ...
        {active && (
          <motion.div
            key="build-banner"
            className="build-panel absolute z-[7] left-1/2 top-[16px] flex items-center gap-[4px] p-[5px] rounded-[16px] surface-sheen shadow-[var(--lift)]"
        ...
        {active && (
          <motion.div
            key="build-tools"
            className="build-panel absolute z-[7] left-[16px] top-1/2 flex flex-col gap-[4px] p-[5px] rounded-[16px] surface-sheen shadow-[var(--lift)]"
        ...
        {active && (
          <motion.div
            key="build-dock"
            className="build-panel absolute z-[7] left-1/2 bottom-[16px] w-[760px] max-w-[calc(100%-160px)] rounded-[20px] surface-sheen shadow-[var(--lift)] overflow-hidden"
          ...
          >
            {tool === "fill" ? (
              <FillColorDock fillColor={fillColor} onSelectFillColor={onSelectFillColor} />
            ) : (
              <>
...
/** Bottom-dock body while the fill tool is active: pick a paint color, THEN
 *  click a raised platform to shade it (or click water, any color, to flood
 *  it into land — the color only matters for the platform case). Swatch
 *  click only updates `fillColor`, unlike the Terrain popover's Island-color
 *  swatches — it never repaints the map itself. */
function FillColorDock({ fillColor, onSelectFillColor }: { fillColor: GrassColor; onSelectFillColor: (c: GrassColor) => void }) {
  return (
    <div className="flex items-center gap-[11px] px-[15px] py-[13px]">
      <span className="text-[11.5px] font-semibold text-txt-2 whitespace-nowrap">Fill color</span>
      <div className="flex items-center gap-[7px]">
        {GRASS_COLOR_LIST.map((c) => {
          const on = fillColor === c.id;
          return (
            <BiomeThumb
              key={c.id}
              def={c}
              size={34}
              onClick={() => onSelectFillColor(c.id)}
              ariaLabel={`Fill color: ${c.label}`}
              selected={on}
              className={`rounded-[9px] cursor-pointer transition-transform duration-150 hover:-translate-y-[2px] ${on ? "shadow-[0_0_0_2px_var(--acc),0_0_0_4px_var(--acc-faint)]" : "shadow-[inset_0_0_0_1px_var(--edge)]"}`}
            />
          );
        })}
      </div>
      <span className="text-[10.5px] font-mono text-txt-4 truncate">click a raised platform to shade it · click water to flood it</span>
    </div>
  );
}
