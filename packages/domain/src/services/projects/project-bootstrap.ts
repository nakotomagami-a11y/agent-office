...
import { expandTilde } from "../infra/paths";
import { readSettings, slugify, scanProjects } from "../settings";
import { ensureDir } from "../infra/fs-atomic";
...
function writeRootClaude(rootDir: string, vars: SubstitutionVars, hasFrontend: boolean, hasBackend: boolean): void {
  ...
  parts.push("");
  parts.push(vars.DESCRIPTION);
  parts.push("");
  parts.push("## Layout");
  parts.push("");
  parts.push("| Directory | Stack | Read |");
  parts.push("|---|---|---|");
  if (hasFrontend) {
    parts.push(`| \`frontend/\` | ${vars.FRONTEND} | \`frontend/CLAUDE.md\` |`);
  }
  if (hasBackend) {
    parts.push(`| \`backend/\` | ${vars.BACKEND} | \`backend/CLAUDE.md\` |`);
  }
  parts.push("");
...
export function bootstrapProject(input: BootstrapInput): BootstrapResult {
  ...
  const vars: SubstitutionVars = {
    ...
    DESCRIPTION: description || "_One sentence: what this does and who uses it._",
  };
