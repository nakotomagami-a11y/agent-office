...
import { isDocCategory, type DocCategory } from "../../config/doc-categories";
import { DOCS_DIR, DOCS_GLOBAL_OWNER, isValidIdSegment } from "../infra/paths";

// ─── Types ───────────────────────────────────────────────────────────────────

import type { Doc, DocMeta, DocFrontmatter } from "../../types/index";
export type { Doc, DocMeta, DocFrontmatter } from "../../types/index";
...
// A docs owner is either an agent-id segment or the `_global` sentinel (which
// fails isValidIdSegment on its leading underscore). Single source of truth for
// this invariant — callers (incl. API routes) should use this, not re-check `_global`.
export function isValidOwner(owner: string): boolean {
  return owner === DOCS_GLOBAL_OWNER || isValidIdSegment(owner);
}
