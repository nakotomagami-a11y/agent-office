...
export const PERMISSION_MODE_META: Record<(typeof PERMISSION_MODE_OPTS)[number], { label: string; subtitle: string }> = {
  bypassPermissions: { label: "Auto", subtitle: "trust all tool calls" },
  default:           { label: "Ask",  subtitle: "prompt on destructive ops — not yet live, denies unattended" },
  plan:              { label: "Deny", subtitle: "read-only mode" },
};
